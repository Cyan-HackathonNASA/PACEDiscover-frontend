...
const Header = () => {
  ...
  return (
    <header className='shadow-lg flex flex-1 w-full justify-between p-6 bg-gray/30 backdrop-blur-md z-50'>
      <a href="/">
        ...
        <ul
          ...
          {listLinks.map(link => (
            <li key={link.label}>
              <a
                href={link.url}
                className={`text-lg font-light text-white hover:text-sky-500 ${
                  pathname === link.url ? 'underline text-sky-500' : ''
                }`}
              >
                {link.label}
              </a>
